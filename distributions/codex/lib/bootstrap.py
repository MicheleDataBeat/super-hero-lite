#!/usr/bin/env python3
"""Total, atomic management of the package-owned Codex bootstrap block."""
...
MARKERS = (
    "<!-- BEGIN SUPER_HERO_LITE_CODEX_BOOTSTRAP v1 -->",
    "<!-- END SUPER_HERO_LITE_CODEX_BOOTSTRAP v1 -->",
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    install_parser.add_argument("agents_path", type=Path)
    install_parser.add_argument("fragment_path", type=Path)
    validate_parser = subparsers.add_parser("validate")
    validate_parser.add_argument("agents_path", type=Path)
    uninstall_parser = subparsers.add_parser("uninstall")
    uninstall_parser.add_argument("agents_path", type=Path)
    arguments = parser.parse_args(argv)

    try:
        text = _read_optional(arguments.agents_path)
        if arguments.command == "install":
            fragment = arguments.fragment_path.read_bytes().decode("utf-8")
            atomic_write(arguments.agents_path, install(text, fragment))
        elif arguments.command == "validate":
            classify(text)
        else:
            result = uninstall(text)
            if result:
                atomic_write(arguments.agents_path, result)
            elif arguments.agents_path.is_symlink():
                atomic_write_bytes(arguments.agents_path, b"")
            elif arguments.agents_path.exists():
                arguments.agents_path.unlink()
